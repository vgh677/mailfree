...
export default {
  ...
  async email(message, env, ctx) {
    ...
    try {
      ...
      // 存储到 KV（原 R2 已替换，绑定名 MAIL_EML 不变）
      const kv = env.MAIL_EML;
      let objectKey = '';
      try {
        const now = new Date();
        ...
        if (kv && rawBuffer) {
          // KV put：直接传 Uint8Array，无需 httpMetadata
          await kv.put(objectKey, new Uint8Array(rawBuffer));
        }
      } catch (e) {
