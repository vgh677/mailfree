 ...
 * @param {object} options - 选项（r2 现在实际是 KV Namespace）
...
export async function handleEmailsApi(request, db, url, path, options) {
  ...
  const kv = options.r2;   // ← 现在是 KV（绑定名 MAIL_EML 不变）
  ...
  if (request.method === 'GET' && path.startsWith('/api/email/') && path.endsWith('/download')) {
    ...
    try {
      ...
      // KV get 直接返回字符串（EML 是文本，完美兼容）
      const emlContent = await kv.get(row.r2_object_key);
      if (!emlContent) return errorResponse('对象不存在', 404);
  ...
  if (request.method === 'GET' && path.startsWith('/api/email/')) {
    ...
    try {
      ...
      // === KV 读取逻辑（原 R2 obj.text / body 已简化）===
      try {
        if (row.r2_object_key && kv) {
          const raw = await kv.get(row.r2_object_key);   // ← KV 直接返回 string 或 null
          if (raw) {
  ...
  // 删除单封邮件（仅删数据库记录，KV 保留原逻辑，如需自动清理可后续加 kv.delete）
